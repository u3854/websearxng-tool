...
async def get_url_content(urls: Union[str, List[str]]) -> Union[str, dict]:
    url_list = [urls] if isinstance(urls, str) else urls
    content_manager = UrlContent(len(url_list))

    for url in url_list:
        # Note: smart_fetch is still synchronous in core.py
    ...
    # Now awaiting the async dump method
    return await content_manager.dump()
...
async def web_search(
    ...
    # search is synchronous in core.py, so we call it normally
    results = agent.search(query)

    if full_content and results:
        urls = [r.url for r in results]
        scraped = await get_url_content(urls)
